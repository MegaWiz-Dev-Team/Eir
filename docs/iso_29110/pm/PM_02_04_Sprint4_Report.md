# PM-02-04: Sprint 4 Report — Yggdrasil JWKS Auth
...
## Sprint Goal
Replace static Bearer token auth with Yggdrasil JWKS-based JWT validation (RS256).
...
| `YggdrasilClaims` — JWT claims model | ✅ Done | `src/jwks.rs` |
| `validate()` — RS256 JWT validation | ✅ Done | `src/jwks.rs` |
| `auth.rs` rewrite — JWKS + static fallback | ✅ Done | `src/auth.rs` |
| `yggdrasil_issuer` + `jwt_audience` config | ✅ Done | `src/config.rs` |
...
## Design Decisions
- **Fallback mode**: if `YGGDRASIL_ISSUER` is empty, falls back to static `AUTH_SECRET`
